...
import { BRAND_FULL, BRAND_WORDMARK } from './brand';
...
function fromAddress() {
  return process.env.EMAIL_FROM ?? `${BRAND_FULL} <no-reply@odatai.com>`;
}
...
<table role="presentation" width="520" cellpadding="0" cellspacing="0" style="max-width:520px;width:100%;">
  <tr><td style="padding:0 8px 18px;">
    <span style="font:700 17px/1 Arial,Helvetica,sans-serif;color:${INK};letter-spacing:.4px;">${BRAND_WORDMARK}</span>
...
export function passwordResetEmail(link: string) {
  return {
    ...
    html: shell(
      'Reset your password',
      p(`We received a request to reset the password for your ${BRAND_FULL} account. Click the button below to choose a new one.`) +
...
export function emailVerificationEmail(code: string, link: string, ttlMinutes: number) {
  return {
    subject: `Verify your ${BRAND_FULL} email`,
    html: shell(
      'Confirm your email',
      ...
      p(`If you didn’t create a ${BRAND_FULL} account, you can safely ignore this email.`),
    ),
    text: `Your ${BRAND_FULL} verification code: ${code}\nOr verify by link: ${link}\nThe code and link expire in ${ttlMinutes} minutes. If you didn't sign up, ignore this email.`,
  };
}

export function welcomeEmail(name: string) {
  return {
    subject: `Welcome to ${BRAND_FULL}`,
    html: shell(
      `Welcome, ${name}!`,
      p(`Your ${BRAND_FULL} account is ready. Order mobile proxies, manage credentials and track renewals from your dashboard.`) +
      cta('Open dashboard', appUrl('/dashboard')) +
      p('Questions? Our support team is one message away on Telegram.'),
    ),
    text: `Welcome to ${BRAND_FULL}, ${name}! Your account is ready: ${appUrl('/dashboard')}`,
  };
}
...
export function passwordChangedEmail() {
  return {
    subject: `Your ${BRAND_FULL} password was changed`,
    html: shell(
      'Password changed',
      p(`The password for your ${BRAND_FULL} account was just changed.`) +
    ...
    text: `Your ${BRAND_FULL} password was changed. If this wasn't you, reset it immediately: ${appUrl('/forgot')}`,
  };
}
