    ...
    >
      ...
      <div style={{ maxHeight: 360, overflowY: 'auto', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)' }}>
        <table className="table" style={{ marginBottom: 0 }}>
          <thead><tr><th></th><th>Proxy</th><th>Carrier · Region</th><th>Pool</th><th>Credentials</th><th>Health</th></tr></thead>
          <tbody>
            {candidates.length === 0
              ...
              : candidates.map(p => (
                <tr key={p.id} onClick={() => toggle(p.id)} style={{ cursor: 'pointer' }}>
                  <td style={{ width: 32 }}>
                    <input type="checkbox" readOnly checked={selected.has(p.id)} disabled={!selected.has(p.id) && selected.size >= qtyNeeded} />
                  </td>
                  <td className="mono td-link">{p.id}</td>
                  <td>{p.carrier} · {p.region}</td>
                  <td>{p.pool}</td>
                  <td className="mono" style={{ fontSize: 12 }}>{p.ip}:{p.port}</td>
                  <td><span className={`chip ${p.health.toLowerCase()}`}>{p.health.toLowerCase()}</span></td>
                </tr>
              ))}
          </tbody>
        </table>
