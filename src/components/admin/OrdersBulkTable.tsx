  ...
  return (
    <>
      ...
      <div className="table-wrap">
        <table className="dt" style={{ minWidth: 1307 }}>
          {/* ATS colgroup, pan-variant (owner: keep every column). B*=1307 =
              min-width — every other anchor keeps its px. Anchors: chk 52,
              Order ID 103, Client 220 (composite id-over-email, see the cell
              below — 220 clears the longest live client email at 11.5px so
              nothing needs a hover), Carrier·Region 162 (nowrap), Amount 98
              ("$9,999.99" + pads), Payment 145 ("Refund requested"), Status
              168 ("Renewal not extended"), Created 133, Expires 138; Plan
              auto (88 at B*). Plain % of 1307. */}
          <colgroup>
            <col style={{ width: '3.9786%' }} />
            <col style={{ width: '7.8807%' }} />
            <col style={{ width: '16.8324%' }} />
            <col />
            <col style={{ width: '12.3948%' }} />
            <col style={{ width: '7.4981%' }} />
            <col style={{ width: '11.0942%' }} />
            <col style={{ width: '12.8539%' }} />
            <col style={{ width: '10.1760%' }} />
            <col style={{ width: '10.5586%' }} />
          </colgroup>
          <thead>
            <tr>
              ...
              <tr key={o.id} style={selected.has(o.id) ? { background: 'var(--accent-subtle)' } : undefined}>
                ...
                <td className="col-id">
                  <div className="client-cell-body">
                    <div className="client-cell-name"><Link href={`/admin/clients/${o.clientId}`} className="td-link">{o.clientId}</Link></div>
                    <div className="client-cell-contact cell-tip" data-tip={o.clientEmail}>{o.clientEmail}</div>
                  </div>
                </td>
